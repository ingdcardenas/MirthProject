# mirthConnect — guía detallada de las herramientas CLI para gestionar canales de Mirth Connect
...
Hace login interactivo (host de `.env`/`MIRTH_URL` o preguntado; usuario
de `.env`/`MIRTH_USER` o preguntado; contraseña siempre con `read -s`) y
persiste la sesión en `MIRTH_COOKIE_JAR` con permisos `600`. Si ya hay una
sesión válida, pregunta si se quiere renovar antes de pedir credenciales
de nuevo.

**Historial de hosts (`.mirth-hosts`)**: cada URL/host contra el que se
loguea queda registrado en un archivo `.mirth-hosts` en la raíz del
repo (una URL por línea, la más reciente primero, sin duplicados —
gitignoreado porque puede contener IPs internas). La resolución de
`MIRTH_URL` sigue esta precedencia:

1. Si `MIRTH_URL` ya viene por entorno/`.env`, se usa tal cual sin
   preguntar nada; igualmente se registra al frente del historial para
   quedar disponible como opción en logins futuros.
2. Si no viene por entorno y ya existe historial, se pregunta de forma
   interactiva mostrando el último host usado como default `[Enter]` y
   la lista numerada de hosts guardados; se puede elegir uno de la lista
   por número, aceptar el default, o escribir un host/URL nuevo.
3. Si todavía no hay historial (primera vez), se pregunta el host con
   default `localhost` (comportamiento histórico del script).

En cualquiera de los casos interactivos, la entrada acepta tanto un
host/IP suelto (se completa automáticamente a `https://host:8443`) como
una URL completa con esquema y puerto propio (`https://host:puerto`), y
el valor resultante se guarda al frente de `.mirth-hosts`.

```bash
./login.sh
# Ejemplo de prompt cuando ya hay historial:
#   Hosts guardados:
#     [1] https://localhost:8443
#     [2] https://otro-servidor.ejemplo.local:8443
#   Host actual: https://localhost:8443 - [Enter] para usarlo, numero de la lista, o host/URL nuevo:
```
...
├── .mirth-hosts         # (generado por login.sh, historial de hosts usados, ignorado por git)
├── login.sh
├── logout.sh
├── test-conexion.sh
├── import-canal.sh
...
- `.env` real, el archivo de sesión y el historial de hosts (`.mirth-hosts`)
  están excluidos vía `.gitignore` — este último porque puede contener
  IPs/nombres de servidores internos usados en logins previos.
